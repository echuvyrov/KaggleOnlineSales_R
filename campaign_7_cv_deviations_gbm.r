...
for(cvFold in 1:1){
  ...
  for( i in 1:12 ) {
    YNaNs <- as.matrix(data[,i])
    YCVTestNaNs <- YNaNs[testRows]
    YCVTrainNaNs <- YNaNs[-testRows]  
    ...
    monthlySalesDeviationFromAverage <- predict.gbm(mo2gbm, newdata=XCVTest, n.trees = ntrees)
    #save monthly sales deviation prediction
    YCVTestPredictedMonthly[,i] <- (monthlySalesDeviationFromAverage + frac[i]) * exp(YCVTestPredictedAnnual)
    
    #now estimate which products will turn NaN (skip first month)
    if(i>1){
      #NaNs appear to depend on Date_1 only (from previous testing)
      XNaNsTrain <- X[-testRows, 2:3]
      XNaNsTest <-  X[testRows, 2:3]
      
      YCVTrainNaNs <- ifelse(is.na(YCVTrainNaNs), 1, 0)
      
      gdata <- data.frame(cbind(YCVTrainNaNs, XNaNsTrain))
      
      mo3gbm <- gbm(YCVTrainNaNs~. ,data=gdata,
                    distribution = "bernoulli",
                    n.trees = ntrees,
                    shrinkage = shrink,
                    cv.folds = folds)
      
      YPredictedNaNs <- predict.gbm(mo3gbm, newdata=XNaNsTest, n.trees = ntrees, type="response")
      
      indexNaNs <- which(YPredictedNaNs > .50)
      YPredictedNaNs[indexNaNs] <- 0
      
      #merge the results of monthly predictions with results of previous (full) prediction
      YCVTestPredictedMonthly[indexNaNs,i] <- 0
