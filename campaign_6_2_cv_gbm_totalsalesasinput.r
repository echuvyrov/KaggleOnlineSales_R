...
YPredictedAnnual <- predict.gbm(gbmAnnual, newdata=Xtest, n.trees = ntrees)
Xtest <- cbind(Xtest, YPredictedAnnual)
...
for(cvFold in 1:10){
  ...
  numberOfRowsToTest <- nrow(XCVTest)
  ...
  for( i in 1:12 ) {  
    ...
    gbmMonthly <- gbm(YCVTrainThisMonth~. ,
                  data=gdata,
                  distribution = "gaussian",
                  n.trees = ntrees,
                  shrinkage = shrink,
    ...
    sqrt(min(gbmMonthly$cv.error))
    which.min(gbmMonthly$cv.error)
    
    summary(gbmMonthly)
    #apply the model to test data
    monthlySales <- predict.gbm(gbmMonthly, newdata=XCVTest, n.trees = ntrees)
    #save monthly sales predictions
    YCVTestPredictedMonthly[,i] <- monthlySales
  }
